...
def get_schedule_for_date(df, target_date):
    ...
    for i, c in enumerate(day_cols):
        ...
        if not subj or subj.lower() == 'nan' or subj.isdigit():
            subj = str(df.iloc[group_row + 1, c]).strip()
        
        if subj and subj.lower() != 'nan' and not subj.replace(' ','').isdigit():
            raw_lessons.append({'idx': i+1, 'subj': subj, 'meta': meta})
        elif i > 0 and raw_lessons:
            prev = raw_lessons[-1]
            raw_lessons.append({'idx': i+1, 'subj': prev['subj'], 'meta': prev['meta']})
    return raw_lessons
